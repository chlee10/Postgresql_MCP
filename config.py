...
- If asked to list tables, query 'user_tables'. If asked for tables with data, SELECT 'table_name, num_rows' and use 'WHERE num_rows > 0'.
...
- **SAFE DATE CONVERSION**: Use CASE WHEN REGEXP_LIKE(col, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') THEN TO_DATE(col, 'YYYYMMDD') ELSE NULL END.
- For tenure/service years: ROUND(MONTHS_BETWEEN(SYSDATE, CASE WHEN REGEXP_LIKE(M.IBSA_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') THEN TO_DATE(M.IBSA_DATE, 'YYYYMMDD') ELSE NULL END) / 12, 1) AS AVG_TENURE (returns NUMBER, not VARCHAR)
- For age calculation: TRUNC(MONTHS_BETWEEN(SYSDATE, CASE WHEN REGEXP_LIKE(M.BRTH_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') THEN TO_DATE(M.BRTH_DATE, 'YYYYMMDD') ELSE NULL END) / 12)
- For age group filtering (40대 이상): REGEXP_LIKE(M.BRTH_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') AND TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(M.BRTH_DATE, 'YYYYMMDD')) / 12) >= 40
- **AGE GROUP QUERIES**: When grouping by age (연령대별), use subquery pattern to avoid GROUP BY issues:
  SELECT AGE_GROUP, COUNT(*) AS EMP_COUNT FROM (
    ...
    FROM (SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, CASE WHEN REGEXP_LIKE(M.BRTH_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') THEN TO_DATE(M.BRTH_DATE, 'YYYYMMDD') ELSE NULL END) / 12) AS AGE FROM INMAST M WHERE REGEXP_LIKE(M.BRTH_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$'))
...
- **TENURE GROUP QUERIES**: When grouping by tenure/service years (근속연수 구간별), use subquery pattern:
  SELECT TENURE_GROUP, COUNT(*) AS EMP_COUNT FROM (
    ...
    FROM (SELECT ROUND(MONTHS_BETWEEN(SYSDATE, CASE WHEN REGEXP_LIKE(M.IBSA_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$') THEN TO_DATE(M.IBSA_DATE, 'YYYYMMDD') ELSE NULL END) / 12, 1) AS TENURE FROM INMAST M WHERE REGEXP_LIKE(M.IBSA_DATE, '^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$'))
